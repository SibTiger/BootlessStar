...
IF EXIST "%~1" (
    CALL "%~1"
    REM Run through the Local Directory protocol
    CALL :UserPreset_Load_LocalDirectoryProtocol
)
GOTO :EOF
...
REM # =============================================================================================
REM # Documentation: Besure that the local directories exists within the specified paths; if not - revert to default to avoid errors.
REM # =============================================================================================
:UserPreset_Load_LocalDirectoryProtocol
IF EXIST "%LocalDirectory.MainRoot%" (
    REM Detected; no problems
    CALL :Initialization_Driver UpdateLocalDirectorySubLocation
    EXIT /B 0
) ELSE (
    CALL :UserPreset_Load_LocalDirectoryProtocol_FaultMSG
...
REM # =============================================================================================
REM # Documentation: If the Local Directory location does NOT exist within the specified filesystem or path, then display that the program is using the default location.
REM # =============================================================================================
:UserPreset_Load_LocalDirectoryProtocol_FaultMSG
ECHO.
ECHO ATTN: The program could not find the local directories at [ %LocalDirectory.MainRoot% ]!  Using default instead...
PAUSE
GOTO :EOF
